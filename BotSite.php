...
class BotSite
{
    ...
    {
        return $this->information[rand(0, count($this->information) - 1)];
    }
    ...
    public function command($key): bool
    {
        ...
        switch ($key) {
            ...
            case 'info':
                $this->textMessage = ('И вот интересный факт:\n' . $this->getInformation());
                $this->buttonMessage = 'Ссылка (статьи на сайте)';
                $this->urlMessage = $this->url . '/articles';
                break;
            ...
            case 'mat':
                ...
                if ($iText == 1) {
                    ...
                    $this->buttonMessage = ((!$this->isVk) ? 'Буду вежлив' : '');
                    $this->urlMessage = ((!$this->isVk) ? ($this->url . '/1') : '');
                } elseif ($iText == 2) {
                    $this->textMessage = 'Эх\nНе хорошо говорить подобные слова\nКак вам не стыдно поступать подобным образом';
                    $this->buttonMessage = ((!$this->isVk) ? 'Прошу прощения' : '');
                    $this->urlMessage = ((!$this->isVk) ? ($this->url . '/2') : '');
                } elseif ($iText == 3) {
                    $this->textMessage = 'А слабо сказать тоже самое только без нецензурных слов😉';
                    $this->buttonMessage = ((!$this->isVk) ? 'Конечно могу' : '');
                    $this->urlMessage = ((!$this->isVk) ? ($this->url . '/3') : '');
                } else {
                    $this->textMessage = 'Мат?!?😳\n Ой все!!!\nНе дружу я с тобой!';
                    $this->buttonMessage = ((!$this->isVk) ? 'Не ойвсекай' : '');
                    $this->urlMessage = ((!$this->isVk) ? ($this->url . '/1') : '');
                }
                break;
